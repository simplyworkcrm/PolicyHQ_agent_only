...
import {
  ArrowDown, ArrowRight, Bot, CalendarDays, Check, CheckCircle2, ChevronRight,
  CircleDollarSign, ClipboardCheck, Loader2, RefreshCw,
  Rocket, Sparkles, Target, TrendingDown, TrendingUp, X,
} from 'lucide-react';
import { useAgentContext } from '../../context/AgentContext';
import { createIncomePlanPublishedSnapshot, getDefaultIncomePlanAssumptions, incomePlanService, publishedIncomePlanApi } from '../../services/incomeGamePlanApi';
...
const Field = ({ label, value, onChange, type = 'number', prefix, suffix, min, max, step }: {
  ...
  <label className="block min-w-0">
    <span className="mb-2 block text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">{label}</span>
...
export const GoalPeriodCard = ({ initial, onContinue }: { initial: IncomePlanAssumptions; onContinue: (values: Partial<IncomePlanAssumptions>) => void }) => {
  ...
  const [values, setValues] = useState({ ...initial, goal_type: 'gross_commission_deposits' as const });
  const set = (key: keyof IncomePlanAssumptions, value: string | number) => setValues(current => ({ ...current, [key]: value }));
  const valid = values.desired_income_goal > 0 && values.expected_working_days > 0 && values.expected_working_days <= 29;
  return (
    <StageCard>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="Desired income goal" prefix="$" value={values.desired_income_goal} min={1} onChange={v => set('desired_income_goal', Number(v))} />
        <Field label="Expected working days" value={values.expected_working_days} min={1} max={29} onChange={v => set('expected_working_days', Math.min(29, Number(v)))} />
      </div>
      <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <PrimaryButton secondary onClick={() => setValues({ ...defaults, goal_type: 'gross_commission_deposits' })}>Use Defaults</PrimaryButton>
...
export const ProductionCard = ({ initial, onContinue }: { initial: IncomePlanAssumptions; onContinue: (values: Partial<IncomePlanAssumptions>) => void }) => {
  ...
  return (
    <StageCard>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="Commission level" suffix="%" value={values.commission_level} min={0} onChange={v => set('commission_level', v)} />
        <Field label="Average annual premium" prefix="$" value={values.average_annual_premium} min={0} onChange={v => set('average_annual_premium', v)} />
        <Field label="Placement rate" suffix="%" value={values.placement_rate} min={0} onChange={v => set('placement_rate', v)} />
      </div>
...
export const ConversionSummaryCard = ({ assumptions, onContinue }: { assumptions: IncomePlanAssumptions; onContinue?: () => void }) => {
  ...
  const metrics = [
    ['Estimated deposit', currencyExact.format(estimatedDeposit), 'Projected from issued policies'],
    ['Submitted applications needed', number.format(submittedApplicationsNeeded), 'Required application volume'],
    ['Placed applications', number.format(placedApplications), 'Expected midpoint'],
    ['Issued policies needed', number.format(issuedPoliciesNeeded), 'Required issued volume'],
  ];

  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-white shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-px bg-slate-100 sm:grid-cols-4">
        {metrics.map(([label, value, helper]) => <div key={label} className="bg-white px-4 py-4"><p className="text-[8px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p><p className="mt-1.5 text-xl font-black tracking-tight text-slate-950">{value}</p><p className="mt-1 text-[9px] font-semibold text-slate-400">{helper}</p></div>)}
      </div>
      {onContinue && <div className="flex justify-end px-4 py-4 sm:px-5"><PrimaryButton onClick={onContinue}>Continue <ArrowRight className="h-3.5 w-3.5" /></PrimaryButton></div>}
    </div>
  );
};
  ...
  return (
    <StageCard>
      <div className="grid grid-cols-1 gap-4">
        <Field label="Marketing cost / submitted app" prefix="$" step={0.01} value={values.marketing_cost_per_application} min={0} onChange={v => set('marketing_cost_per_application', v)} />
      </div>
...
export const ReviewCard = ({ assumptions, onBuild }: { assumptions: IncomePlanAssumptions; onBuild: () => void }) => {
  const sections = [
  ...
  return <StageCard>
    ...
    <div className="mt-5 flex justify-end"><PrimaryButton onClick={onBuild}><Sparkles className="h-4 w-4 text-amber-300" /> Build My Plan</PrimaryButton></div>
  </StageCard>;
};
...
export const ResultsCard = ({ results, assumptions, publishedSnapshot, onContinue, aiFeedback, aiFeedbackLoading = false, aiFeedbackUnavailable = false }: {
  ...
  const monthlyApTarget = publishedSnapshot?.monthly_ap_target ?? Math.ceil(assumptions.average_annual_premium * workingDays);
  const dailyApTarget = publishedSnapshot?.daily_ap_target ?? Math.ceil(monthlyApTarget / workingDays);
  ...
  return <StageCard>
    ...
    <div className="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-white">
      ...
      <div className="grid grid-cols-1 divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
        <div className="p-4"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-500">Daily target</p><div className="mt-3 space-y-3">
          ...
          <div className="flex items-end justify-between gap-4 border-t border-slate-100 pt-3"><div><p className="text-[9px] font-bold text-slate-500">AP target</p><p className="mt-0.5 text-[8px] font-semibold text-slate-400">Monthly AP ÷ {workingDays} days</p></div><p className="text-lg font-black text-slate-950">{currency.format(dailyApTarget)}</p></div>
    ...
    {onContinue && <div className="mt-4 flex justify-end"><PrimaryButton onClick={onContinue}>Review commitment <ArrowRight className="h-3.5 w-3.5" /></PrimaryButton></div>}
  </StageCard>;
};
...
export const CommitmentCard = ({ onRestart, onPublish }: { onRestart: () => void; onPublish: () => void }) => (
  <StageCard>
    <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-end">
      <PrimaryButton secondary onClick={onRestart}><RefreshCw className="h-4 w-4" /> Restart Plan</PrimaryButton>
...
export const IncomeGamePlanPage = () => {
  ...
  const boot = async () => {
    ...
    setLoading(true); setError(null); setConversation([]); setResults(null); setPublishedPlan(null);
    try {
  ...
  const generateAiFeedback = async (itemId: string, planResults: IncomePlanResults, planAssumptions: IncomePlanAssumptions) => {
    ...
    const monthlyApTarget = Math.ceil(planAssumptions.average_annual_premium * workingDays);
    ...
    const dailyApTarget = Math.ceil(monthlyApTarget / workingDays);
    const resultBrief = [
  ...
  const publish = async () => {
    ...
    try {
      ...
      const official = await publishedIncomePlanApi.publish(snapshot);
  ...
  const restartPlan = () => {
    const restartItem: ConversationItem = {
      id: `goal-period-restart-${Date.now()}`,
      stage: 'goal_period',
      assistant_message: 'Let’s revisit your goal. I kept your previous answers in place so you can adjust only what needs to change.',
  ...
  if (publishedPlan && results) return <div className="mx-auto max-w-4xl"><div className="mb-5 flex items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-emerald-600">Official Income Game Plan</p><h1 className="mt-1 text-2xl font-black text-slate-950">Your locked-in plan</h1><p className="mt-1 text-[10px] font-semibold text-slate-400">This plan has been published and cannot be submitted again.</p></div><button type="button" onClick={() => navigate('/business/overview')} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-[10px] font-black text-slate-600">Back to My Business</button></div><ResultsCard results={results} assumptions={assumptions} publishedSnapshot={publishedPlan} aiFeedback={publishedPlan.ai_feedback} /></div>;

  return <div className="mx-auto grid h-[calc(100dvh-6rem)] min-h-0 max-w-6xl grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_240px]">
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-[2rem] border border-white bg-white shadow-sm">
      ...
      <div role="log" aria-live="polite" className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain bg-slate-50/50 px-4 py-5 sm:px-6">
        {conversation.map((item, index) => <div key={item.id} className="animate-in fade-in slide-in-from-bottom-2 duration-300"><div className="flex items-start gap-3"><span className="mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-700"><Sparkles className="h-3.5 w-3.5" /></span><div className="min-w-0 max-w-[760px] flex-1"><p className="whitespace-pre-line text-sm font-semibold leading-6 text-slate-700">{item.assistant_message}</p>{item.values && <CompletedAnswerCard stage={item.stage} values={item.values} />}{item.stage === 'conversion_summary' && index < conversation.length - 1 && <ConversionSummaryCard assumptions={item.accepted_values as IncomePlanAssumptions} />}{item.stage === 'results' && (item.results || results) && <ResultsCard results={(item.results || results) as IncomePlanResults} assumptions={{ ...assumptions, ...(item.accepted_values || {}) }} aiFeedback={item.aiFeedback} aiFeedbackLoading={item.aiFeedbackLoading} aiFeedbackUnavailable={item.aiFeedbackUnavailable} onContinue={index === conversation.length - 1 && stage === 'results' ? () => void sendTurn() : undefined} />}{index === conversation.length - 1 && <>
          ...
          {stage === 'goal_period' && <GoalPeriodCard initial={assumptions} onContinue={values => void sendTurn(values)} />}
          {stage === 'production_profile' && <ProductionCard initial={assumptions} onContinue={values => void sendTurn(values)} />}
          {stage === 'conversion_summary' && <ConversionSummaryCard assumptions={assumptions} onContinue={() => void sendTurn()} />}
          {stage === 'business_costs' && <BusinessCostsCard initial={assumptions} onContinue={values => void sendTurn(values)} />}
          {stage === 'review' && <ReviewCard assumptions={assumptions} onBuild={() => void sendTurn()} />}
          {stage === 'commitment' && <CommitmentCard onRestart={restartPlan} onPublish={() => setConfirmPublish(true)} />}
        </>}</div></div></div>)}
