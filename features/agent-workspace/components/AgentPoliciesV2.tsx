  ...
  return (
    <div className={`animate-in fade-in duration-300 ${isDownlineVariant ? 'rounded-[2rem] border border-amber-100/80 bg-[#fffaf0] p-5 shadow-inner shadow-amber-900/5' : ''}`}>
      ...
      <div className={`relative flex min-w-0 flex-1 flex-col overflow-hidden border bg-white shadow-sm ${attentionOnly ? 'min-h-[480px] rounded-[2rem] border-white' : 'min-h-[600px]'} ${isDownlineVariant ? 'rounded-[1.75rem] border-amber-100' : attentionOnly ? '' : 'rounded-[2.5rem] border-slate-100'}`}>
          ...
          <div>
            {displayItems.map((policy, idx) => {
              ...
              <div
                onClick={() => {
                  if (!readOnlyRows) {
                    setDetailModalPolicyId(isSelected ? null : policy.policy_id);
        ...
        {/* ── Policy Detail Panel ──────────────────────────────────────── */}
        {!readOnlyRows && (
        <div className={`shrink-0 self-start transition-all duration-300 ease-in-out ${
          selectedPolicy
            ? detailModalPolicyId
              ? 'w-1/3 min-w-0 max-w-[32rem] opacity-100'
              : 'w-1/3 min-w-0 max-w-[32rem] opacity-100'
            : 'w-0 opacity-0 pointer-events-none'
        }`}>
          {selectedPolicy && (
            detailModalPolicyId ? (
              <div className="w-full rounded-3xl border border-white/70 bg-slate-100 shadow-sm">
                <AgentPolicyDetails
